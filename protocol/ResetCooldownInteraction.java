   ...
   public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
      if (buffer.readableBytes() - offset < 43) {
         ...
         if ((nullBits & 32) != 0) {
            ...
            int posxxxxx = offset + 43 + cooldownOffset;
            if (posxxxxx >= buffer.writerIndex()) {
               return ValidationResult.error("Offset out of bounds for Cooldown");
            }

            ValidationResult cooldownResult = InteractionCooldown.validateStructure(buffer, posxxxxx);
            if (!cooldownResult.isValid()) {
               return ValidationResult.error("Invalid Cooldown: " + cooldownResult.error());
            }

            posxxxxx += InteractionCooldown.computeBytesConsumed(buffer, posxxxxx);
         }
